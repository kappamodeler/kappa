...
let pprint_ODE_middle2 print print_fragment get_fragment ode_handler views_data_structures keep_this_link aux_file jac_file init_file obs_file file_ODE_data file_ODE_XML   nfrag nobs is_obs pb_obs flag_map = 
  ...
  let _ = 
	...
	begin
	  ...
	  in
          let _ =
	    if !Config_complx.ode_output_mode = "MATLAB" 
            then 
              a.print_string ("soln =  ode15s(@"^(Tools.cut aux_file)^",[tinit tend],"^(Tools.cut init_file)^"(),options);\nsoln.y=soln.y\n")
            else
...
(*  let _ = pprint_string print_main ("function "^Tools.cut file_main^"\n\n") in *)
  let _ = pprint_string print_main ("% This file should be post-processed with "^(!Config_complx.ode_output_mode)^"\n") in 
  ...
  let _ = pprint_newline print_main in 
  let _ = pprint_newline print_main in
(*  let _ = pprint_string print_main ("mode='"^(match !Config_complx.ode_output_mode with "MATLAB" -> "MATLAB" | _ -> "OCTAVE")^"' %existing modes are 'OCTAVE' and 'MATLAB', wrong string will be considered as 'OCTAVE'\n\n") in *)
