...
let pprint_ODE_middle2 print aux_file jac_file init_file obs_file file_ODE_data file_ODE_XML   nfrag nobs is_obs = 
  let _ = 
    match print.mathematica with 
      None -> () 
            ...
            if file_ODE_XML<>"" then 
              begin 
                ...
                let _ = 
                  ...
                  in vide 1 in 
                let _ = a.print_string ("fputs(fid,\"<CSV>\\n<![CDATA[\\n\");\n") in
                let _ = 
                  a.print_string "fputs(fid,\"]]>\\n</CSV>\\n</Simulation>;\\n\");" 
                in
                let _ = 
                  a.print_string "fputs(fid,\"</ComplxSession>\\n\");\nfclose(fid);";
                in ()
              end 
          in 
	    ()
...
let pprint_ODE_head print print_obs print_activity file file_jac file_size file_activity file_obs = 
 ...
 let dump_prod (prod,jac) init obs (init_t,final,step) print_ODE_mathematica print_ODE_matlab print_ODE_matlab_aux print_ODE_matlab_jac print_ODE_matlab_size output_data file_aux file_jac  file_init file_obs file_data file_XML size  nobs is_obs = 
...
(*   let _ = 
   ...
   let _ = pprint_ODE_middle2 print_ODE file_jac file_aux file_init file_obs file_data file_XML nfragments nobs is_obs in 
   let _ = 
     List.fold_left
       (fun bool c -> 
	  let _ = if bool then pprint_eq_separator print_ODE in 
  ...
  let _ = pprint_ODE_middle2 print_ODE file_aux file_jac file_init file_obs file_data file_XML nfragments nobs is_obs in 
