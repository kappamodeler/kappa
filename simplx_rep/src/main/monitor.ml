...
let rec apply sd c p log =
  match sd.task_list with
      (t,tsk)::tl -> 
	if t >= c.curr_time then 
	  if c.deadlock then 
	    apply sd {c with curr_time = t ; deadlock = false} p 
	      (Session.add_log_entry 1 (Printf.sprintf "-Task list not empty, moving time to %f" t) log) 
	  else
	  ...
	  begin
		...
		SAVE_MIXTURE -> 
		  ...
		  let d = open_out_bin file in
		    ...
		    close_out d ;
		    apply {sd with task_list = tl} c p log
		  ...
		  begin
		    ...
		    in
		      match opt with
			  None -> apply {sd with task_list = tl} c p log
			| Some pert_i ->
			    ...
			    in
			      apply {sd with lab = lab ; task_list = tl} c p log
		  end
		  ...
		  begin
		    ...
		    in
		      match opt with
			  None -> apply {sd with task_list = tl} c p log (*perturbation already removed*)
			| Some pert_i ->
		  ...
		  in
		    apply {sd with task_list = tl} {c with snapshot_counter = c.snapshot_counter+1} p log
